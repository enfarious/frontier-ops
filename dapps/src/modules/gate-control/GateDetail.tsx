...
export function GateDetail({ gateId, assembly }: GateDetailProps) {
  const { data, isLoading, error, refetch } = useGateState(gateId);
  const { bringOnline, bringOffline, rename, isPending } = useAssemblyActions();

  const [newName, setNewName] = useState("");
  const [status, setStatus] = useState<string | null>(null);

  if (isLoading) return <Text>Loading gate details...</Text>;
  if (error) return <Text color="red">Error loading gate: {String(error)}</Text>;
  ...
  async function handleToggleOnline() {
    ...
    try {
      if (isOnline) {
        ...
        setStatus("Gate taken offline");
      } else {
        await bringOnline(assemblyInfo);
        setStatus("Gate brought online");
      }
    ...
    <Flex direction="column" gap="4">
      <Card>
        <Flex direction="column" gap="3">
          <Heading size="3">{currentName || "Gate"}</Heading>
          <Separator size="4" />
          ...
          <Flex gap="4" wrap="wrap">
            ...
            <Box>
              ...
              <Code size="1" variant="ghost">{gateId.slice(0, 16)}...{gateId.slice(-8)}</Code>
            </Box>
          </Flex>
      ...
      <AccessListManager assemblyId={gateId} assemblyLabel="gate" />
    </Flex>
  );
}
